...
from moe_model import MoEModel  # import your hybrid model
...
def evaluate_model(model_name, ckpt_path, data_root, out_dir, device):
    ...
    # if evaluating hybrid model
    if model_name.lower() == "proposed_moe":
        model = MoEModel(
        ...
        print(f"[Info] Loaded hybrid MoE model from {ckpt_path}")
    else:
    ...
    with torch.no_grad():
        for i, (imgs, labels) in enumerate(tqdm(loader)):
            imgs = imgs.to(device)
            outs = model(imgs)["logits"] if model_name.lower() == "proposed_moe" else model(imgs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--models", type=str, default="proposed_moe")
    parser.add_argument("--ckpt-dir", required=True)
    parser.add_argument("--data-root", required=True)
    parser.add_argument("--out-dir", default="../multi_results")
    args = parser.parse_args()

    device = torch.device("cpu")  # Force CPU for this run
    os.makedirs(args.out_dir, exist_ok=True)
    model_names = [m.strip() for m in args.models.split(",")]

    summary = {}
    for m in model_names:
        if m.lower() == "proposed_moe":
            ckpt = os.path.join(args.ckpt_dir, "moe_finetuned.pth")
        else:
            ckpt = os.path.join(args.ckpt_dir, m, f"{m}_best.pth")
