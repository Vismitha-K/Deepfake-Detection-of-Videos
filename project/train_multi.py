...
# ==============================================================
#                 GOOGLE DRIVE SAFETY SETUP (Fast + Reliable)
# ==============================================================

import subprocess, os, shutil

USE_DRIVE = True   # Still True, but now zip-based instead of copytree

if USE_DRIVE:
    DRIVE_ROOT = "/content/drive/MyDrive/deepfake-detection-videos"
    DRIVE_ZIP = os.path.join(DRIVE_ROOT, "celebdf_frames.zip")
    ...
    if not os.path.exists(LOCAL_DATA) or len(os.listdir(LOCAL_DATA)) == 0:
        ...
        if not os.path.exists(DRIVE_ZIP):
            raise FileNotFoundError(f"❌ Could not find celebdf_frames.zip in {DRIVE_ROOT}")
        try:
            subprocess.run(["unzip", "-q", DRIVE_ZIP, "-d", "/content/"], check=True)
            print("✅ Dataset extracted to local SSD.")
        except subprocess.CalledProcessError:
            raise RuntimeError("❌ Failed to extract celebdf_frames.zip from Drive.")
    else:
        print("✅ Local dataset already exists, skipping extraction.")

    # --- Output + checkpoint directories on Drive ---
...
def get_model(name, num_classes=2, pretrained=True):
    ...
    if name == "resnet50":
        ...
        # The backbone feature extractor outputs 960 channels
        # Replace the full classifier with a simple dropout + linear head
